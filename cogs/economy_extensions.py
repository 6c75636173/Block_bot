...
from core.database import check_cooldown, set_cooldown
...
marriages = load_data(MARRIAGES_FILE)
gangs = load_data(GANGS_FILE)
game_stats = load_data(STATS_FILE)
    ...
    async def gang_leave(interaction: discord.Interaction):
        ...
        if user_gang["leader"] == user_id:
            await interaction.response.send_message(
                "❌ Le leader ne peut pas quitter son propre gang pour l'instant (dissoudre un gang n'existe pas encore comme fonctionnalité). Transmets le lead à un autre membre ou contacte un admin.",
                ephemeral=True
            )
            return
...
async def setup_economy_commands(bot, users_data, save_users_data):
    ...
    async def work(interaction: discord.Interaction):
        ...
        emoji = "💼"
        bonus_data = {"name": "Travail", "work_bonus": 1.0}
    ...
    async def crime(interaction: discord.Interaction):
        ...
        emoji = "🚔"
        bonus_data = {"name": "Crime", "crime_bonus": 1.0}
    ...
    async def heist(interaction: discord.Interaction, complice1: discord.Member, complice2: discord.Member = None):
        ...
        pending = {str(c.id): None for c in complices}
...
async def setup(bot):
    ...
    fun_group = app_commands.Group(name="fun", description="Commandes de divertissement")
    bot.tree.add_command(fun_group)
    await setup_marriage_commands(bot, core.users_data, save_users_data)
    await setup_gang_commands(bot, core.users_data, save_users_data)
    await setup_achievement_commands(bot, core.users_data)
    await setup_economy_commands(bot, core.users_data, save_users_data)
    await setup_misc_commands(bot, core.users_data, save_users_data, fun_group)